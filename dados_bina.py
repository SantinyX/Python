...
def main():
    ...
    with open("copia_img.bin", "wb") as copy_file:
        copy_file.write(data)

        # Manipulação dos dados binários copiados
        # Exemplo: Inverter os bytes
        with open("copia_img.bin", "rb") as file:
            data = bytearray(file.read())

        with open("copia_img.bin", "wb") as file:
            file.write(data)

        # Carregar e mostrar a imagem manipulada
        modified_data = np.frombuffer(
            data, dtype=np.uint8).reshape(img_data.shape)
