# Exemplos de manipulação de arquivos em Python utilzando split, join, read, write, etc.
def zenit_polar_replace(text):
    # Aplicando a codificação ZENIT POLAR utilizando replace
    replacements = [('z', 'p'), ('e', 'o'), ('n', 'l'), ('i', 'a'), ('t', 'r'),
                    ('Z', 'P'), ('E', 'O'), ('N', 'L'), ('I', 'A'), ('T', 'R')]
    for old, new in replacements:
        text = text.replace(old, new)
    return text


def main():
    # Entrada da frase e aplicação da codificação
    phrase = "The quick brown fox jumps over the lazy dog."
    phrase = phrase.title()  # Coloca a frase em formato título

    # Dividir a frase em palavras
    words = phrase.split()

    # Processar cada palavra na lista
    coded_words = [zenit_polar_replace(word) for word in words]

    # Juntar as palavras de volta em uma frase
    coded_phrase = ' '.join(coded_words)
    print("Frase original:", phrase)
    print("Frase codificada:", coded_phrase)


if __name__ == "__main__":
    main()
